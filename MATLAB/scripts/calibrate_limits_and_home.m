...
% calibrate_limits_only_keep_home
% Manual calibration (limits only):
%  - torque OFF
%  - for each motor: move to MIN extreme (ENTER), then MAX extreme (ENTER)
%  - coupled pair (2,3) calibrated together
%  - HOME is NOT re-captured: we keep previous rawHome from calibration file if available
%
% Outputs:
...
% Per your description: IDs 2&3 are MX64AT, the rest AX-12A
isAX = [true false false true true true];

% MoveIt joint order [base shoulder elbow wrist ee] maps to servo IDs:
moveitToServoID = [1 2 4 5 6];

% Direction signs for MoveIt joints (keep your current defaults)
...
if exist(matPath, "file")
    S = load(matPath, "cal");
    if isfield(S, "cal") && isfield(S.cal, "rawHome") && numel(S.cal.rawHome) == 6
        rawHome = double(S.cal.rawHome(:));
        fprintf("\nKeeping existing HOME raw from previous calibration file:\n");
        for i = 1:6
...
if isempty(rawHome)
    ...
    fprintf("\nNo previous HOME found. Using CURRENT raw as rawHome (no movement):\n");
    for i = 1:6
        fprintf("  ID%d: %d\n", ids(i), round(rawHome(i)));
    end
end
...
fprintf(fid, "%% Auto-generated by calibrate_limits_only_keep_home.m on %s\n\n", cal.createdAt);
